...
static pthread_barrier_t *lock_wait;
static pthread_barrier_t *lock_go;
...
/*
 * Lower priority processes will just grab one lock
 * and wait to release it.
 */
static void local_lock(long id, int iter)
{
	ftrace_write("thread %ld iter %d, taking its lock\n",
		     id, iter);
	pthread_mutex_lock(&locks[id]);
	ftrace_write("thread %ld iter %d, waiting to wake up\n",
		     id, iter);
	pthread_barrier_wait(&lock_wait[id]);
	ftrace_write("thread %ld iter %d, releasing its lock\n",
		     id, iter);
	pthread_mutex_unlock(&locks[id]);
	/* wait for the lock to be released again */
	pthread_barrier_wait(&lock_go[id]);
}
...
static void grab_lock(long id, int iter, int l)
{
	...
	/* tell the user we want the lock */
	ftrace_write("thread %ld iter %d, waiting to wake up on %d\n",
		     id, iter, l);
	pthread_barrier_wait(&lock_wait[l]);
	...
	ftrace_write("thread %ld iter %d, took lock %d in %llu us\n",
		     id, iter, l, (time - try_time) / 1000);

	pthread_mutex_unlock(&locks[l]);
	pthread_barrier_wait(&lock_go[l]);
}
...
int main (int argc, char **argv)
{
	...
	if (!lock_time)
		perr("malloc");
	memset(lock_time, 0, sizeof(*lock_time) * nr_locks);

	lock_wait = malloc(sizeof(*lock_wait) * nr_locks);
	if (!lock_wait)
		perr("malloc");
	for (i = 0; i < nr_locks; i++) {
		ret = pthread_barrier_init(&lock_wait[i], NULL, 2);
		if (ret < 0)
			perr("pthread_barrier_init");
	}

	lock_go = malloc(sizeof(*lock_go) * nr_locks);
	if (!lock_go)
		perr("malloc");
	for (i = 0; i < nr_locks; i++) {
		ret = pthread_barrier_init(&lock_go[i], NULL, 2);
		if (ret < 0)
			perr("pthread_barrier_init");
	}
